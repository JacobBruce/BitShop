...
function trim_decimal($dec, $len) {
  $dec_pos = strpos($dec, '.');
  if ($dec_pos !== false) {
    return substr($dec, 0, $dec_pos+$len+1);
  } else {
    return $dec;
  }
}
...
function list_binaries($directory) {
  if (is_dir($directory)) {
...
// get the total or remaining disk space
function get_disk_space($type='total') {
  if ($type === 'total') {
    return round(disk_total_space('/') / 1024 / 1024 / 1024, 2);
  } else {
    return round(disk_free_space('/') / 1024 / 1024 / 1024, 2);
  }
}

function url_query_str() {
